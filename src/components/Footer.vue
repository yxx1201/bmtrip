...
<div style="width: 100vw; height:1.6rem;"></div>
  <footer class="main">
    <Tabbar
      ...
      placeholder
      z-index:999
      active-color="rgb(177, 28, 145)"
      inactive-color="#666"
      @change="FiconTab($event)"
      router
    >
      <TabbarItem
        icon-prefix="my-icon"
        v-for="(item, index) in state.iconArr"
        :key="index"
        :to="item.path"
        :icon="item.isX ? item.icon1 : item.icon"
        >{{ item.name }}
      </TabbarItem>
    </Tabbar>
  </footer>
...
function FiconTab(e) {
  state.iconArr.forEach((i) => {
    i.isX = false;
  });
  state.iconArr[e].isX = true;
}
</script>
...
.van-tabbar--fixed {
  height: 1.2rem;
  box-shadow: 0 -0.05rem 0.6rem 0 #00000020;
}
</style>
